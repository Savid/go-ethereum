...
type Config struct {
	...
	EnableActualGas  bool // enable actual gas cost computation from consecutive gas values (non-streaming mode only)
...
type StructLogger struct {
	...
	jsonLogs      []json.RawMessage // buffer of json-encoded logs (when EnableActualGas is false)
	logs          []*StructLog      // buffer of logs for deferred JSON encoding (when EnableActualGas is true)
...
func NewStreamingStructLogger(cfg *Config, writer io.Writer) *StructLogger {
	...
	l.writer = writer
	if cfg != nil && cfg.EnableActualGas {
		log.Warn("EnableActualGas is not supported in streaming mode, ignoring")
		l.cfg.EnableActualGas = false
		l.pendingGasIdx = nil
	}
	return l
}
