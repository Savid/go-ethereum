...
// TestEnableActualGas verifies that when EnableActualGas is true, gasCost is
...
func TestEnableActualGas(t *testing.T) {
	t.Run("same_depth", func(t *testing.T) {
		// Create logger with EnableActualGas enabled
		logger := NewStructLogger(&Config{EnableActualGas: true})
		evm := vm.NewEVM(vm.BlockContext{}, &dummyStatedb{}, params.TestChainConfig, vm.Config{Tracer: logger.Hooks()})
		contract := vm.NewContract(common.Address{}, common.Address{}, new(uint256.Int), 100000, nil)
	...
	t.Run("disabled", func(t *testing.T) {
		// Default behavior - EnableActualGas is false
		logger := NewStructLogger(nil)
		evm := vm.NewEVM(vm.BlockContext{}, &dummyStatedb{}, params.TestChainConfig, vm.Config{Tracer: logger.Hooks()})
		contract := vm.NewContract(common.Address{}, common.Address{}, new(uint256.Int), 100000, nil)
	...
	t.Run("streaming_mode_ignores_flag", func(t *testing.T) {
		// Verify that EnableActualGas is ignored in streaming mode
		var buf bytes.Buffer
		logger := NewStreamingStructLogger(&Config{EnableActualGas: true}, &buf)
